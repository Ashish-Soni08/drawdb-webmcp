  ...
  return {
    tables: [
      {
        id: "u",
        name: "users",
        fields: [f("u1", "id", { primary: true }), f("u2", "email")],
        indices: [
      ...
      },
      {
        id: "i",
        name: "invoices",
        fields: [f("i1", "id", { primary: true }), f("i2", "sub_id")],
        indices: [],
        uniqueConstraints: [],
      },
    ],
    relationships: [
      ...
      },
      {
        ...
        endTableId: "s",
        endFieldId: "s1",
        fields: [{ startFieldId: "i2", endFieldId: "s1" }],
      },
    ],
  };
}
...
test("removing a table cascades to its relationships and reports the impact", () => {
  ...
  assert.deepEqual(
    r.impact.relationships.map((x) => x.name),
    ["fk_subs_user"],
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    r.next.relationships.map((x) => x.name),
    ["fk_inv_sub"],
  );
  assert.match(
    summarizeRemoval(r.impact),
    /1 table\(s\): users; 1 relationship/,
  );
});

test("removing a field drops dependent relationships and prunes indexes", () => {
  const r = planRemoval(
    { targets: [{ kind: "field", table: "subs", field: "user_id" }] },
    diagram(),
  );
  assert.equal(r.ok, true);
  ...
  assert.deepEqual(
    subs.fields.map((x) => x.name),
    ["id", "plan"],
  );
  assert.deepEqual(
    subs.indices[0].fields,
    ["plan"],
    "index keeps its remaining column",
  );
  assert.equal(r.impact.indexes.length, 0);

  const r2 = planRemoval(
    { targets: [{ kind: "field", table: "users", field: "email" }] },
    diagram(),
  );
  assert.deepEqual(
    r2.impact.indexes,
    [{ table: "users", index: "users_email_idx" }],
    "index with no columns left is dropped",
  );
});
...
test("relationship and index targets, and validation errors", () => {
  const r = planRemoval(
    {
      targets: [
        { kind: "relationship", name: "fk_inv_sub" },
        { kind: "index", table: "subs", name: "subs_user_plan_idx" },
      ],
    },
    diagram(),
  );
  ...
  const bad = planRemoval(
    {
      targets: [
        ...
        { kind: "column", table: "users" },
        { kind: "field", table: "users", field: "nope" },
      ],
    },
    diagram(),
  );
