...
export function reviewSchema(diagram, validatorIssues = []) {
  ...
  for (const suggestion of suggestFixes(diagram)) {
    findings.push(
      finding(
        ...
        suggestion.operation.op === "add_index"
          ? "fk_without_index"
          : "fixable",
        suggestion.issue,
        {
  ...
  for (const r of relationships) {
    for (const pair of getRelationshipFields(r))
      fkFieldIds.add(pair.startFieldId);
  }
  ...
  for (const table of tables) {
    ...
    if (!SNAKE_CASE.test(table.name)) {
      findings.push(
        finding(
          "hint",
          "naming",
          `Table "${table.name}" is not snake_case; mixed naming styles make generated SQL harder to use.`,
          {
            table: table.name,
          },
        ),
    ...
    if (tables.length > 1 && !linkedTableIds.has(table.id)) {
      findings.push(
        finding(
          "hint",
          "isolated_table",
          `Table "${table.name}" has no relationships to any other table.`,
          {
            table: table.name,
          },
        ),
    ...
    if (!fields.some((f) => TIMESTAMP_NAMES.includes(f.name.toLowerCase()))) {
      const timestampType = types.TIMESTAMP
        ? "TIMESTAMP"
        : types.DATETIME
          ? "DATETIME"
          : null;
      findings.push(
        finding(
          "hint",
          "no_created_at",
          `Table "${table.name}" has no creation timestamp column.`,
          {
            table: table.name,
            ...(timestampType && {
              fix: {
      ...
      if (!SNAKE_CASE.test(field.name)) {
        findings.push(
          finding(
            "hint",
            "naming",
            `Column "${table.name}.${field.name}" is not snake_case.`,
            {
              table: table.name,
              field: field.name,
            },
      ...
      if (meta?.isSized && (field.size === undefined || field.size === "")) {
        findings.push(
          finding(
            "warning",
            "unsized_type",
            `Column "${table.name}.${field.name}" is ${field.type} without a length.`,
            {
              table: table.name,
              field: field.name,
            },
      ...
      if (fkFieldIds.has(field.id) && !field.notNull && !field.primary) {
        findings.push(
          finding(
            "hint",
            "nullable_fk",
            `Foreign key column "${table.name}.${field.name}" allows NULL; make it NOT NULL if the relationship is mandatory.`,
            {
              table: table.name,
              field: field.name,
              fix: {
