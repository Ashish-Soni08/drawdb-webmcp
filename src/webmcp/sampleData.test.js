...
const diagram = {
  ...
  tables: [
    ...
    },
    {
      ...
      name: "subs",
      fields: [
        f("s1", "id", "INTEGER", { primary: true }),
        f("s2", "user_id", "INTEGER"),
        f("s3", "status", "VARCHAR", { size: 6 }),
        f("s4", "started", "TIMESTAMP"),
      ],
    },
    {
      id: "u",
      name: "users",
      fields: [
        f("u1", "id", "INTEGER", { primary: true }),
        f("u2", "email", "VARCHAR", { size: 255 }),
      ],
    },
    {
      id: "x",
      name: "audit",
      fields: [
        f("x1", "id", "INTEGER", { primary: true }),
        f("x2", "kind", "ENUM", { values: ["a", "b"] }),
      ],
    },
  ],
  relationships: [
...
test("generates deterministic inserts with valid foreign keys", () => {
  ...
  assert.match(
    r.sql,
    /INSERT INTO "subs"[\s\S]*\(1, 1, 'stat1', CURRENT_TIMESTAMP\)/,
    "FK points at row 1; VARCHAR(6) is truncated",
  );
...
test("validates options and supports table filters and dialect quoting", () => {
  ...
  const only = generateSampleInserts(
    { ...diagram, database: "mysql" },
    { tables: ["USERS"] },
  );
...
test("explainJoinPath finds the shortest FK chain in either direction", () => {
  ...
  assert.deepEqual(r.hops, [
    {
      ...
      to: { table: "subs", field: "user_id" },
      via: "fk_subs_user",
    },
    {
      from: { table: "subs", field: "id" },
      to: { table: "invoices", field: "sub_id" },
      via: "fk_inv_sub",
    },
  ]);
  assert.equal(
  ...
  assert.equal(
    explainJoinPath(diagram, { from: "users", to: "ghost" }).ok,
    false,
  );
  assert.equal(
    explainJoinPath(diagram, { from: "users", to: "users" }).ok,
    false,
  );
});
